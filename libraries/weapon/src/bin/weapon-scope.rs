...
fn main() {
    ...
    use weapon::opfs::{parse_device_counts, parse_event_log_records, EventLogRecord};
        ...
        for (device_id, device_records) in events_by_device {
            ...
            for (i, record) in device_records.iter().enumerate() {
                ...
                if index < expected_index {
                    ...
                    println!("    ⚠️  Event {}: Index {} (BACKTRACKING - expected >= {})", i, index, expected_index);
                } else if index > expected_index {
                    has_gaps = true;
                    println!("    ⚠️  Event {}: Index {} (GAP - expected {})", i, index, expected_index);
                } else {
                    println!("    Event {}: Index {}", i, index);
                }
                ...
                let event_str = serde_json::to_string(&record.event.event).unwrap_or_else(|_| "Invalid JSON".to_string());
    ...
    for record in &records {
        *parsed_device_counts.entry(record.device_id.clone()).or_default() += 1;
    }
    ...
    for (device_id, expected_count) in &device_counts {
        ...
        if actual_count != *expected_count {
            println!("  ❌ Count mismatch for device {}: header says {} but found {} events", 
                     device_id, expected_count, actual_count);
            has_mismatch = true;
        }
    }
    ...
    for (device_id, count) in &parsed_device_counts {
        if !device_counts.contains_key(device_id) {
            println!("  ⚠️  Device {} has {} events but is not in device counts", device_id, count);
        }
    }
}