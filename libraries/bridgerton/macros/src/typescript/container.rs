...
use super::{attrs::ContainerAttrs, error_tracker::ErrorTracker};

/// Data structure storing information about a transparent bridged type.
/// This structure also keeps information that was parsed via Serde's macros.
pub struct Container<'a> {
    /// Errors that occurred during processing.
    pub errors: ErrorTracker,
    /// Declaration options from the bridge attribute.
...
impl<'a> Container<'a> {
    pub fn new(serde_container: SerdeContainer<'a>, attrs: ContainerAttrs) -> Self {
    ...
    pub fn from_derive_input(
        ...
        match serde_cont {
            Some(serde_container) => {
                cx.check()?;
                Ok(Self::new(serde_container, attrs))
            }
    ...
    pub fn serde_data(&self) -> &ast::Data<'a> {
