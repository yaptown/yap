...
- Whenever you change a TTS voice or model (e.g. `GEMINI_TTS_MODEL`, a Cloud TTS voice, a new provider), check the output by hand before shipping: synthesize a single word and a sentence in a few course languages, and confirm the clip durations are plausible and a Whisper transcript contains only the requested words. Also listen to at least one clip. Model upgrades change behavior silently. In 2026-09 `gemini-3.8-flash-tts` started reading our direction line aloud and returning WAV instead of raw PCM, which played as static. The automated Whisper gate missed it because it skips most languages and exact-matches only multi-word text, and unit tests can't catch it either.
